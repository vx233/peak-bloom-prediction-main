...
kyoto_year_temps <-
  ...
  mutate(`very cold` = jan_temp < 4.6 & feb_temp < 4.6,
         `cold` = jan_temp < 4.6, feb_diff = feb_temp - jan_temp, mar_diff = mar_temp - jan_temp,
         `temp level` = factor(c(ifelse(`cold` == T, "cold", "warm"))), 
         grp = factor(c(ifelse(mar_diff < 4.7, "g1",ifelse(mar_diff < 6.0,'g2','g3')))))
...
kyoto2 <- kyoto1[-16,] %>% mutate(threshold = threshold_100_days1)
...
as.Date(87, origin = "2023-01-01")
```

There is a moderate threshold at 100 days and the curve is similar in appearance to , so the predicted bloom date for Kyoto, Japan this year is April 9, 2023, at 98 days.
...
Bloom date is 98 days: April 9, 2023
